...
EIPmean= ongoing.condition[,1]#M parameter
EIPsd=1
slopeMos=ongoing.condition[,2]#B parameter
slopeMos.sd=0.2
Pmax.mos.putative=ongoing.condition[,3]#K parameter
Pmax.mos.var=0.002
Pmax.mos=Pmax.mos.putative-Pmax.mos.var
  #Beta law parameters
    alpha.Mos=(Pmax.mos*(1-Pmax.mos)/Pmax.mos.var)*Pmax.mos
    beta.Mos=(alpha.Mos/Pmax.mos)-alpha.Mos
...
birth.date = c(rep(0,(Total.mos.in.sim)), final)

table1.vect = data.table(ID.vect,birth.date)
...
table1.vect$EIP50 = rnorm(Total.mos.in.sim,EIPmean,EIPsd) #M parameter
table1.vect$slopeInfec =  rtnorm(Total.mos.in.sim,slopeMos,slopeMos.sd, a=0, b=Inf)
table1.vect$Pmax = rbeta(Total.mos.in.sim,alpha.Mos,beta.Mos)
...
for (st in 1:nrow(events)){
  ...
  if(Minfected == TRUE && Vinfected == FALSE){
    ...
    b.Pmax = table1.vect[b.ongoing, Pmax]
    b.EIP50 = table1.vect[b.ongoing, EIP50]
    b.slope = table1.vect[b.ongoing, slopeInfec]

    Ptransmit.t.vect2 = viral.transmission.mos(x, b.Pmax , b.EIP50 , b.slope)
    Ptransmit.t.vect = ifelse(Ptransmit.t.vect2 > 0, Ptransmit.t.vect2, 0)
...
stopCluster(cl)
write.table(Outbreak_indices, file=paste("EIP",ongoing.condition[,1],"_slope",ongoing.condition[,2],"_Pmax",ongoing.condition[,3],"_simulation.txt", sep=""), quote = F, sep = "\t", row.names = T)
}
