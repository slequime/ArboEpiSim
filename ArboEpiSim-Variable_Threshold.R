...
for(i in 1:Total.mos.in.sim){
  ...
  if(u == 1) {
    result[i] = rlogis(1, location = EIPmean, scale = 1/slopeMos)
  }
  else if(u == 0) {
    result[i] = Inf
  }
...
stopCluster(cl)
write.table(Outbreak_indices, file=paste("EIP",ongoing.condition[,1],"_slope",ongoing.condition[,2],"_Pmax",ongoing.condition[,3],"_variable_threshold_simulation.txt", sep=""), quote = F, sep = "\t", row.names = T)
}